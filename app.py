...
# --------------------------------------------
def get_custom_plotly_layout_args(height: int, title_text: str) -> dict:
    """
    Returns a dict of common Plotly layout arguments for dark-themed charts.
    """
    return {
        "title": {"text": title_text, "font": {"color": ACCENT_COLOR}},
        "height": height,
...
# ------------------- Title -------------------
st.markdown(
    "<h1 style='text-align: center; margin-bottom:0.5rem;'>🌬️ AuraVision AQI</h1>",
    unsafe_allow_html=True
)
st.markdown(
    f"<p style='text-align: center; color: {SUBTLE_TEXT_COLOR_DARK_THEME}; font-size: 1.1rem; margin-bottom: 2.5rem;'>"
    "Illuminating Air Quality Insights Across India</p>",
    unsafe_allow_html=True
)
...
def load_data_and_metadata():
    with st.spinner("🌬️ Initializing AuraVision Engine... Please wait."):
        today = date.today()
...
if data_last_updated:
    st.caption(
        ...
        f"{load_message} | Last data update: {data_last_updated.strftime('%Y-%m-%d %H:%M:%S')}</p>",
        unsafe_allow_html=True
    )
else:
    st.caption(
...
selected_month_name = st.sidebar.selectbox(
    "🌙 Select Month (Optional)", month_options_list, index=0
)

month_number_filter = None
if selected_month_name != "All Months":
    month_number_filter = next(k for k, v in months_map_dict.items() if v == selected_month_name)
...
if not selected_cities:
    ...
    for city in selected_cities:
        ...
        tab_trend, tab_dist, tab_heatmap_detail = st.tabs([
            "📊 TRENDS & CALENDAR", 
            "📈 DISTRIBUTIONS", 
            "🗓️ DETAILED HEATMAP"
        ])
        ...
        with tab_trend:
            ...
            fig_cal = go.Figure(
                data=go.Heatmap(
                    ...
                    colorscale=[[i / (len(CATEGORY_COLORS_DARK) - 1), color] 
                                for i, color in enumerate(CATEGORY_COLORS_DARK.values())],
                    showscale=False,
            ...
            city_data_trend["rolling_avg_7day"] = city_data_trend["index"].rolling(
            ...
            fig_trend_roll.add_trace(
                go.Scatter(
                    x=city_data_trend["date"], 
                    y=city_data_trend["index"],
                    mode="lines+markers", 
                    name="Daily AQI",
            ...
            fig_trend_roll.add_trace(
                go.Scatter(
                    x=city_data_trend["date"], 
                    y=city_data_trend["rolling_avg_7day"],
                    mode="lines", 
        ...
        with tab_dist:
            ...
            city_data_full["month_name"] = pd.Categorical(
                city_data_full["month_name"], categories=month_order_cat, ordered=True
            )
        ...
        with tab_heatmap_detail:
            ...
            heatmap_pivot = city_data_full.pivot_table(
                index="month_name",
...
with st.container():
    ...
    if not pollutant_data_A.empty:
        ...
        percent_poll_A = grouped_poll_A.apply(
            lambda x: x / x.sum() * 100 if x.sum() > 0 else x, axis=1
        ).fillna(0)
        percent_poll_A_long = percent_poll_A.reset_index().melt(
...
with st.container():
    st.markdown(
        f"#### ⛽ Dominant Pollutants for Selected Period ({selected_month_name}, {year})"
    )
    ...
    if not pollutant_data_B.empty and "pollutant" in pollutant_data_B.columns:
        ...
        grouped_poll_B["percentage"] = (
            grouped_poll_B["count"] / total_days_B * 100
            if total_days_B > 0 else 0
        ).round(1)
...
with st.container():
    ...
    if len(forecast_src_data) >= 15:
        ...
        if len(forecast_df) >= 2:
            ...
            future_dates_list = [
                min_date_forecast + timedelta(days=int(i)) for i in future_X_range
            ]

            plot_df_obs = pd.DataFrame({"date": forecast_df["date"], "AQI": y_train})
            plot_df_fcst = pd.DataFrame({
                "date": future_dates_list,
                "AQI": np.maximum(0, future_y_pred)
            })
        ...
        st.warning(
            f"Need at least 15 data points for {forecast_city_select} for forecasting; found {len(forecast_src_data)}."
        )
...
with st.container():
    ...
    try:
        if os.path.exists(coords_file_path):
            ...
            local_scope = {}
            exec(file_content, {}, local_scope)  # Execute lat_long.txt to obtain `city_coords`
    ...
    if not df_period_filtered.empty:
        ...
        if city_coords_data and not map_grouped_data.empty:
            ...
            if not latlong_map_df.empty:
                ...
                if not map_merged_df.empty:
                    # Assign AQI Category based on average
                    map_merged_df["AQI Category"] = map_merged_df["avg_aqi"].apply(
                        lambda val: next(
                            ...
                            }.items() if v_range[0] <= val <= v_range[1]),
                            "Unknown"
                        )
                    ...
                    fig_scatter_map = px.scatter_mapbox(
                        ...
                        lat="lat",
                        lon="lon",
                        size=np.maximum(map_merged_df["avg_aqi"], 1),  # Ensure size ≥ 1
                        size_max=25,
                        color="AQI Category",
                        color_discrete_map=CATEGORY_COLORS_DARK,
                        hover_name="city",
                    ...
                    # Use our helper to populate baseline layout arguments
        ...
        if not scatter_map_rendered:
            # Fallback if scatter map can't render
            ...
            if not map_grouped_data.empty:
                ...
                # Ensure the y-axis is ordered ascending
